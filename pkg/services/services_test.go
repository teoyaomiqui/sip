...
var _ = Describe("Service Set", func() {
	Context("When new SIP cluster is created", func() {
		It("Deploys services", func() {
			By("Getting machine IPs and creating secrets and pods")

			bmh1, _ := testutil.CreateBMH(1, "default", "control-plain", 1)
			bmh2, _ := testutil.CreateBMH(2, "default", "control-plain", 2)
			m1 := &vbmh.Machine{
				BMH: *bmh1,
			...
			sip := testutil.CreateSIPCluster("my-sip-cluster", "default", 1, 1)
